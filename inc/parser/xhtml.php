...
class Doku_Renderer_xhtml extends Doku_Renderer {
    ...
    var $toc_html = '';   // will contain the HTML of Table of Contents
    ...
    function document_end() {
        ...
        if($this->info['toc']){
// vot
//            $this->toc_html = $this->render_TOC();
//echo "parser/xhtml:Doku_Renderer_xhtml started: this->toc_html prepared.<HR>";
// vot            $this->doc = $this->render_TOC().$this->doc;
//            $this->doc = $this->get_right_banner();
	    
            $this->doc = $this->render_TOC().
			 $this->get_right_banner().
			 $this->get_right_link_block().
			 $this->doc;
        }
    }

    /**
     * Return the Right Link Block rendered to XHTML
     *
     * @author vot
     */
    function get_right_link_block(){
        global $conf, $ID, $ACT;

	$out = "";

       if($conf['start'] == $ID && $ACT == 'show') {

        $banner = "";
        $banner .= @file_get_contents ($conf['datadir'].'/site/rightlinkblock.txt');

//print "file=".$conf['datadir'].'/site/rightbanner.txt'. "<br>";
//print "banner=".$banner. "<br>";

        $banner = trim($banner);
        if(!strlen($banner)) return '';

        $out .= '<div class="toc">'.DOKU_LF;
        $out .= '<div class="right_banner">';
        $out .= $banner;
    ...
    /**
     * Return the Right Banner text rendered to XHTML
     *
     * @author vot
     */
    function get_right_banner(){
        global $conf, $ID, $ACT;
...
//print "get_right_banner() started. <br>";
        $banner = "";
        $banner .= @file_get_contents ($conf['datadir'].'/site/rightbanner.txt');
