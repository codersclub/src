...
// vot        $hsplit = preg_split( '/(={2,})/u', $match,-1,
        $hsplit = preg_split( '/(={2,})/', $match,-1,
...
// vot       $link = preg_replace(array('/^\[\[/','/\]\]$/u'),'',$match);
        $link = preg_replace(array('/^\[\[/','/\]\]$/'),'',$match);

        // Split title from URL
// vot        $link = preg_split('/\|/u',$link,2);
        $link = preg_split('/\|/',$link,2);
        if ( !isset($link[1]) ) {
            $link[1] = NULL;
