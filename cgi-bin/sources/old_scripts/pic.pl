...
foreach $StrokaZakazaFull(@lines) {
   ...
   if($StrokaZakaza[0] eq $Id) {
	...
	if($Count eq "1") {
		...
		print "<a href=$CGIURL/pic.pl?id=",$Id-1,">&lt;&lt;&lt; Назад</a>";
	}

	print " <b>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;</b> 
		<a href=$CGIURL/pic.pl?id=",$Id+1,"> Вперед &gt;&gt;&gt;</a></p>
...
for($i=1; $i<$Count; $i++) {
   if($i eq $Id) {
	...
	if($i < 10) {
	   print "[<a href=\"$CGIURL/pic.pl?id=$i\">0$i</a>]";
	}
	else {
	   print "[<a href=\"$CGIURL/pic.pl?id=$i\">$i</a>]";
	}
   }
}
