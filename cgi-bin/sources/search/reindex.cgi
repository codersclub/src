...
#  $url= lc($url);
  ...
  $html_text = <FILE>;
  ($title) = ($html_text =~ m#<TITLE>\s*(.*?)\s*</TITLE>#is);
  if ($title eq "") {$title = "No title"};
  if ($use_META eq "YES") {
    ($keywords,$description) = &get_META_info($html_text);
  }
  if ($use_ALT eq "YES") {
...
#    ($plain_text = $html_text) =~ s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
  ...
  $plain_text = $decription." ".$plain_text." ".$alt;
  $plain_text =~ s/\s+/ /gs;
  if ($use_esc eq "YES") { $plain_text =~ s/(&.*?;)/&esc2char($1)/egs; }
#  $descript = substr($plain_text,0,256);
  $chars = "a-zA-Z".$CAP_LETTERS.$LOW_LETTERS.$numbers;
  @wwd = ($plain_text =~ m/([$chars]+-[$chars-]+[$chars]+)/gs);
  $wwd = join " ", @wwd;
  $plain_text =~ s/[^$chars]/ /gs;
  ...
  if (!$description) {
    $description = substr($plain_text,0,256);
  }
  if (substr($description,0,1) eq "'") {
    $description = substr($description,1);
  }
  if (substr($description,length($description)-1,1) eq "'") {
    $description = substr($description,0,length($description)-1);
  }
  $fileinfo = $url."::".$size."::".$title."::".$description;
  $pos = tell(FINFO);
