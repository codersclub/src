...
//	echo "<h1>Apache configuring in progress...</h1>Please, come back later!";
//	exit();
...
define( 'ROOT_PATH', "./" );
define( 'BASE_DIR', "/home/forum/htdocs/TEST/" );


$file = __FILE__;
$dirname= dirname(__FILE__);
$the_script = str_replace($dirname,"",$file);
$the_script = preg_replace("#^(\\\\|/)#","",$the_script);
...
require BASE_DIR."conf_global.php";



//--------------------------------
// Redefine the $INFO for the site homepage!
//--------------------------------

require "./qconf_site.php";
...
//require BASE_DIR$INFO['base_dir']."sources/functions.php";
require BASE_DIR."sources/functions.php";
...
//$to_require = $INFO['base_dir']."sources/Drivers/".$INFO['sql_driver'].".php";
$to_require = BASE_DIR."sources/Drivers/".$INFO['sql_driver'].".php";
require ($to_require);
...
//require $INFO['base_dir']."sources/store/store_functions.php";
require BASE_DIR."sources/store/store_functions.php";
...
// vot } else $ibforums->input['act'] = $ibforums->input['act'] == '' ? "idx" : $ibforums->input['act'];
} else $ibforums->input['act'] = $ibforums->input['act'] == '' ? $ibforums->vars['default_action'] : $ibforums->input['act'];

//foreach($ibforums->vars['default_action'] as $k=>$v) {
...
//	$ibforums->base_url   = ROOT_PATH.'index.'.$ibforums->vars['php_ext'].'?';
	$ibforums->base_url   = ROOT_PATH.$the_script.'?';
} else
{
//	$ibforums->base_url = $ibforums->vars['board_url'].'/index.'.$ibforums->vars['php_ext'].'?s='.$ibforums->session_id.'&amp;';
//	$ibforums->base_url = ROOT_PATH.'index.'.$ibforums->vars['php_ext'].'?s='.$ibforums->session_id.'&amp;';
	$ibforums->base_url = ROOT_PATH.$the_script.'?s='.$ibforums->session_id.'&amp;';
}
...
//$ibforums->js_base_url = ROOT_PATH.'/index.'.$ibforums->vars['php_ext'].'?s='.$ibforums->session_id.'&';
$ibforums->js_base_url = $ibforums->base_url;

// Debug
//echo $ibforums->base_url."<br>";
//echo $ibforums->vars[home_url]."<br>";
...
if ($ibforums->input['act'] != 'Login' and $ibforums->input['act'] != 'Reg' and $ibforums->input['act'] != 'Attach')
{
	...
	{
		require $INFO['base_dir']."sources/Login.php";
	}
}
...
if ( $ibforums->input['act'] == 'home' )
{
	...
	{
		require $INFO['base_dir']."sources/dynamiclite/csite.php";
		$csite = new click_site();
	} else
	{
		require $INFO['base_dir']."sources/Boards.php";
	}
...
} elseif ( $ibforums->input['act'] == 'module' )
{
	if ( USE_MODULES == 1 )
	{
		require $INFO['base_dir']."modules/module_loader.php";
...
//{
//echo "Require: ".$INFO['base_dir']."sources/".$choice[ $ibforums->input['act'] ].".php<br>";
//if (file_exists($INFO['base_dir']."sources/".$choice[ $ibforums->input['act'] ].".php")) 
//{
//echo "File: ".$INFO['base_dir']."sources/".$choice[ $ibforums->input['act'] ].".php Exists<br>";
//}
//}
     require $INFO['base_dir']."sources/".$choice[ $ibforums->input['act'] ].".php";
}
